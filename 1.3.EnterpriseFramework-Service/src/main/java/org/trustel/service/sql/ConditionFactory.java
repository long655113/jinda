	...
	protected void where(String fieldName, int[] values, int action) {
		...
		if (action == QueryAction.IN || action == QueryAction.NOT_IN) {
			if (values != null && values.length > 0) {
				...
				if(action==QueryAction.NOT_IN){
					conditionBuffer.append(" not in");
				}else if(action==QueryAction.IN){
					conditionBuffer.append(" in");
				}
				conditionBuffer.append("  (").append(stmp.toString()).append(")");
				append(att2FName(fieldName) + conditionBuffer.toString(),true);
			}
		} else
